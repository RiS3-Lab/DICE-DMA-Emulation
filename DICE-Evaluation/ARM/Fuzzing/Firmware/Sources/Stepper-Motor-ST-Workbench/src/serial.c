...
//#define TX_BUFFER_SIZE (4*1024) alejandro the buffer is too big for fuzzing requires too much input
...
void Serial_InitRxSequence(void) {
  while(HAL_UART_Receive_DMA(&huart2, rxBuffer, RX_BUFFER_SIZE) == HAL_BUSY)
  {
	  __HAL_UNLOCK(&huart2);
  }
  serialStatus |= SERIAL_RX;
