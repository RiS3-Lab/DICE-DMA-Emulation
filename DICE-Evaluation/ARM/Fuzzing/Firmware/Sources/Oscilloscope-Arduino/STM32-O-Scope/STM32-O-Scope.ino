...
struct dPlot 
{ 
uint16_t canary3[128];
uint16_t dataPlot[320]; //max(width,height) for this display
uint16_t canary4[128];
};

dPlot sdPlot;

uint16_t *dataPlot = (uint16_t *)&sdPlot.dataPlot;
...
void setup()
{
  ...
  //for(i=0;i<sizeof(canary1);i++)canary1[i]=i;
  //for(i=0;i<sizeof(dataPoints32);i++)dataPoints32[i]=i;
  //for(i=0;i<sizeof(canary2);i++)canary2[i]=i;
  //for(i=0;i<sizeof(canary3);i++)canary3[i]=i;
  //for(i=0;i<sizeof(dataPlot);i++)dataPlot[i]=i;
  //for(i=0;i<sizeof(canary4);i++)canary4[i]=i;
...
void serialSamples ()
{
  ...
  Serial.println("#Time(uS), ADC Number, value, diff");
  for (int16_t j = 1; j < maxSamples   ; j++ )
  {
    // Time from trigger in milliseconds
    Serial.print((samplingTime / (maxSamples))*j);
    Serial.print(" ");
    // raw ADC data
    Serial.print(j % 2 + 1);
    Serial.print(" ");
    Serial.print(dataPoints[j] );
    Serial.print(" ");
    Serial.print(dataPoints[j] - dataPoints[j - 1]);
    Serial.print(" ");
    Serial.print(dataPoints[j] - ((dataPoints[j] - dataPoints[j - 1]) / 2));
    Serial.print("\n");
...
void toggleHold()
{
  ...
  if (triggerHeld)
  {
    Serial.println("# Toggle Hold on");
  }
  else
  {
    Serial.println("# Toggle Hold off");
  }
}

void toggleSerial() {
  serialOutput = !serialOutput ;
  Serial.println("# Toggle Serial");
  serialSamples();
}
...
void toggleTestPulseOn () {
  ...
  Serial.println("# Test Pulse On.");
}

void toggleTestPulseOff () {
  pinMode(TEST_WAVE_PIN, INPUT);
  Serial.println("# Test Pulse Off.");
}
...
void readTouchCalibrationCoordinates()
{
  ...
  Serial.println(tx / thisCount, HEX);
  Serial.print("# Calib y: ");
  Serial.println(ty / thisCount, HEX);
