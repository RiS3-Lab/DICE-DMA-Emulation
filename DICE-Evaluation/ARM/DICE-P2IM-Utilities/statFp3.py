...
outF: optional parameter, the file where the detailed register categorization result is written to.
...
def getStat(model,groundT,outF=""):
   ...
   if (outF != ""):
      with open(outF+".csv", 'w') as csvfile:
...
if __name__== "__main__":
   if len(sys.argv)==4:
      ...
      if outF != "":
         aux['SRSITES'] = list(aux['SRSITES'])
         aux['INTERRUPTS'] = list(aux['INTERRUPTS'])
